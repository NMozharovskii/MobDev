...
import { Text, View, StyleSheet, Image, Button, Platform, TouchableOpacity, FlatList } from 'react-native';
...
function AppMain() {
  ...
  return (
    <View style={styles.container}>
      {selectedUri != null && <Image style={{ height: 200, borderRadius: 5 }} source={{ uri: selectedUri }} />}
      {gifIds.map((item, index) => (
        <TouchableOpacity key={item} onPress={() => handleSelect(item)} style={{ marginTop: "5%", borderWidth: 1, borderRadius: 5, padding: 10 }}>
          <Text style={{ textAlign: 'center' }}>Gif #{index + 1}</Text>
          <Text>{selectedUri}</Text>
        </TouchableOpacity>
      ))}

      <TouchableOpacity title="Unload all" onPress={unloadAll} style={{ marginTop: "5%", borderWidth: 1, borderRadius: 5, padding: 10, backgroundColor: "blue" }}>
        <Text style={{ textAlign: 'center', color: "white" }}>Закрыть гифку</Text>
      </TouchableOpacity>
    </View>
  );
}
