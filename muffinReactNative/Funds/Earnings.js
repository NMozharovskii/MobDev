...
export default function Earnings() {
    ...
    const getData = () => {
        setIsLoading(true)
        let URL = 'http://192.168.231.132:8000/earnings'
        fetch(URL, {
            headers: {
                'Token': token
            }
    ...
    function deleteData(id) {
        ...
        fetch('http://192.168.231.132:8000/earnings/'+id, requestOptions).then((res) => {
            return res.json();
        }).then((res) => {
            getData()
    ...
    function sendData() {
        ...
        if (currentAmount !== '') {
            fetch('http://192.168.231.132:8000/earnings', requestOptions).then((res) => {
                res.json()
                return res;
            }).then((res) => {
                getData()
